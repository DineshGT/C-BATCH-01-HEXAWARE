...
Select Products.prod_name, Products.Category 
From Products
Group by prod_name, Category;
...
Select SUM(Inventory.quantityInStock) AS Total_Available_products 
From Inventory;

--3. Write an SQL query to calculate the total revenue generated by TechShop.

Select SUM(orders.total_amnt) AS Total_revenue From orders;
...
-- there is no category column in any table give, so lets add it first
...
Update Products
Set Category = 'Gaming'
where Products.prod_id = 11; -- for 1, 6, 9, and 11
...
Select (Select SUM(total_amnt)
        From orders
        Where cust_id = c.cust_id) AS total_revenue
From customers c
Where c.cust_id = 104;

--Alternate join approach
Select Sum(order_details.quantity * Products.prod_price) As Total_revenue
From order_details
Join orders on order_details.order_id = orders.order_id
Join Customers on orders.cust_id = Customers.cust_id
Join Products on Products.prod_id = order_details.prod_id
Where Customers.cust_id = 105;
...
Select first_name, 
             (Select total_amnt From Orders where orders.cust_id = Customers.cust_id) As Total_revenue
From Customers
Order By Total_revenue DESC;
