...
using static ElectronicGadgetsTechShop.Exceptions.ExceptionHandling;
...
namespace ElectronicGadgetsTechShop
{
    public class DataBaseConnector
    {
        ...
        {
            con = new SqlConnection("data source = DESKTOP-FKC1CCN\\SQLEXPRESS01;initial catalog = TechShop;integrated security = true;");
            con.Open();
            return con;
        }
        ...
        public static bool InsertCustomer(Customers customer)
        {
            ...
            try
            {
                using (SqlConnection conn = getConnection())
                {
                    ...
                    // we need to check if the email already exists.. because it is set with unique constraint in the table..

                    string checkquery = "Select count(*) From Customers Where cust_email = @Email";

                    // here i use "using" which helps not to worry about closing and opening connections...
                    ...
                    using (SqlCommand checkcmd = new SqlCommand(checkquery, conn))
                    {
                        ...
                        if (count > 0)
                        {
                            Console.WriteLine("There is a customer with same email detected!");
                            return false;
                        }
                    }
        ...
        public static List<Orders> GetAllOrders()
{
    ...
    return orders;
        ...
        public static bool ProcessingOrder(Orders order)
        {
        ...
        public static List<string> GetCustomerOrderStatuses(int cust_id)
        {
        ...
        public static bool InsertOrderDetail(OrderDetails detail)
        {
            using (SqlConnection conn = getConnection())
            {
        ...
        public static bool InsertInventory(Inventory inventory)
        {
            try
            {
                using (SqlConnection conn = getConnection())
