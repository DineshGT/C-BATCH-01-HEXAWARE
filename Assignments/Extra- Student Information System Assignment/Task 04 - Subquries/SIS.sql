...
-- Task 04 : Subquery and its types

--1. Write an SQL query to calculate the average number of students enrolled in each course. Use 
--aggregate functions and subqueries to achieve this. 

Select avg(student_count) As Average
From (Select course_id, Count(std_id) As student_count From Enrollments Group by course_id) As student_count;
...
Select *
From Students
Where Students.std_id in (Select std_id From Payments Where Payments.amount in 
...
where course_id in (Select course_id From Enrollments
						Group by course_id 
						Having count(std_id) = (Select Max(enrolled_count) From 
						                        (Select count(std_id) As enrolled_count From Enrollments
												    Group by course_id) As enrolled_count));
...
Select * From Students
Where Students.std_id in (Select Payments.std_id From Payments 
                           Group by std_id Having count(Payments.payment_id) > 1  );
...
Select Students.std_id, Students.first_name, Sum(Payments.amount) As Total_payments
From Students
Left Join Payments
On Students.std_id = Payments.std_id
Group by Students.std_id, Students.first_name;
...
Select Courses.course_name, Count(Enrollments.std_id) As Count_of_students
From Courses
...
Select Students.first_name, avg(Payments.amount) As Average
From students
Join Payments 
On Students.std_id = Payments.std_id
